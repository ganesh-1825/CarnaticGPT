...
def get_audio_for_raga(raga_name: str) -> Dict:
    """Wrapper that always returns a dict with a 'found' key for server.py."""
    data = get_raga_audio(raga_name)
    if data is None:
        return {"found": False, "raga": raga_name, "audio_files": {}, "available_types": []}
...
def get_audio_index() -> Dict[str, Dict]:
    """Return the full mapping of all ragas to their audio data."""
    _mapper.load()
    return {key: ra.to_dict() for key, ra in _mapper._map.items()}
