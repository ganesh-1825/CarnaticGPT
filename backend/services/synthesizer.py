...
def get_api_keys() -> tuple[str | None, str | None]:
    ...
    # Fallback to reading .env file directly to pick up runtime edits
    try:
        from pathlib import Path
        _root = Path(__file__).resolve().parent
...
def _call_gemini(query: str, chunks: list[dict], key: str) -> str:
    import requests
    parts = []
    ...
    system_prompt = (
        ...
        "Your task is to answer the user's question exactly and perfectly using ONLY the provided retrieved context. "
        "Extract direct references, details, and page numbers from the sources where appropriate.\n"
        "STRICT INSTRUCTIONS:\n"
        "1. Rely ONLY on the provided RETRIEVED CONTEXT.\n"
        "2. Do not use any outside knowledge. Do not assume or extrapolate beyond the context.\n"
        "3. Do not invent or hallucinate facts.\n"
        "4. If the retrieved context does not contain the answer, reply exactly with:\n"
    ...
    for model in ["gemini-2.5-flash", "gemini-1.5-flash", "gemini-pro"]:
        for domain in ["generativelanguage.googleapis.com", "generativetool.googleapis.com"]:
            try:
                url = f"https://{domain}/v1beta/models/{model}:generateContent?key={key}"
                data = {
                    "contents": [{
                        "parts": [{
                ...
                log.warning(f"Gemini API ({model} on {domain}) failed: {e}")
    raise Exception("All Gemini API endpoints failed.")


def _call_openai(query: str, chunks: list[dict], key: str) -> str:
    import requests
    parts = []
    for i, c in enumerate(chunks[:8], 1):
        m = c.get("metadata") or {}
    ...
    if res.status_code == 200:
        ...
        if text.strip():
            return text.strip()
    raise Exception(f"OpenAI API query failed with status code {res.status_code}: {res.text}")
...
def _synthesize_raw(
    ...
    # ── Cloud LLM Prioritization (if key is present and evidence score is high) ──
    if use_llm:
        ...
        if score >= 35.0:
            if gemini_key:
                try:
                    answer = _call_gemini(query, chunks, gemini_key)
                    if _is_real_answer(answer):
                        log.info("synthesis_method=gemini (top)  len=%d", len(answer))
                        return answer, "gemini"
                except Exception as e:
                    log.warning("Gemini API call (top) failed: %s", e)
            if openai_key:
                try:
                    answer = _call_openai(query, chunks, openai_key)
                    if _is_real_answer(answer):
                        log.info("synthesis_method=openai (top)  len=%d", len(answer))
                        return answer, "openai"
                except Exception as e:
                    log.warning("OpenAI API call (top) failed: %s", e)

    # ── LIST_RAGAS_BY_SCALE Interceptor ──────────────────────────────────────
    # ── LIST_RAGAS_BY_SCALE Interceptor ──────────────────────────────────────
    if intent == "LIST_RAGAS_BY_SCALE":
        target_types = []
        if "shadava" in query_lower or "hexatonic" in query_lower:
            target_types.append("Shadava")
        if "audava" in query_lower or "pentatonic" in query_lower:
            target_types.append("Audava")
        if "sampurna" in query_lower or "sampoorna" in query_lower or "heptatonic" in query_lower:
            target_types.append("Sampurna")
        ...
        def get_scale_count(scale_str: str) -> int:
            if not scale_str:
                return 0
            notes = [x.strip() for x in re.split(r'\s+', scale_str.strip()) if x.strip()]
            if not notes:
                return 0
            if len(notes) > 1 and notes[-1].upper().startswith("S"):
                notes = notes[:-1]
            return len(notes)
            
        classified = {"Audava": [], "Shadava": [], "Sampurna": [], "Asymmetric": []}
        
        # Five special focus ragas for advanced classification
        special_names = ["bhairavi", "anandabhairavi", "kamboji", "kambhoji", "bilahari", "arabhi"]
        
        for name, r_info in RAGA_KNOWLEDGE_BASE.items():
            aro_count = get_scale_count(r_info.get("arohana", ""))
            ava_count = get_scale_count(r_info.get("avarohana", ""))
            
            r_name_clean = r_info.get("name", "").lower()
            
            # Check if it belongs to our curated special list
            is_special = any(x in r_name_clean for x in special_names)
            
            if is_special:
                classified["Asymmetric"].append(r_info)
            elif aro_count == 5 and ava_count == 5:
                classified["Audava"].append(r_info)
            elif aro_count == 6 and ava_count == 6:
                classified["Shadava"].append(r_info)
            elif aro_count == 7 and ava_count == 7:
                classified["Sampurna"].append(r_info)
            else:
                classified["Asymmetric"].append(r_info)
                
        SPECIAL_RAGAS_NOTES = {
            "bhairavi": (
                "**Bhairavi** is a majestic Bhashanga raga. While it is heptatonic (Sampurna) in both movements, "
                "it features a crucial microtonal asymmetry: it uses **Chatusruti Dhaivata (D2)** in the ascending scale (arohana) "
                "and **Suddha Dhaivata (D1)** in the descending scale (avarohana). This makes its scale structure highly unique and fluid."
            ),
            "anandabhairavi": (
                "**Anandabhairavi** features a **Vakra (zigzag)** ascending scale (`S G2 R2 G2 M1 P D2 P S`), which utilizes "
                "non-linear movements, and a Sampurna descending scale. It is a traditional Bhashanga raga with deep emotional resonance."
            ),
            "kamboji": (
                "**Kamboji** is a classic **Shadava-Sampurna** raga. It omits Nishadha (Ni) in the ascending scale "
                "(6 notes: `S R2 G3 M1 P D2 S`) but includes Kaisiki Nishadha (N2) in the descending scale (7 notes: `S N2 D2 P M1 G3 R2 S`)."
            ),
            "bilahari": (
                "**Bilahari** is an **Audava-Sampurna** raga. Its ascending scale is pentatonic (5 notes: `S R2 G3 P D2 S`), "
                "omitting Ma and Ni, while its descending scale is fully heptatonic (7 notes: `S N3 D2 P M1 G3 R2 S`)."
            ),
            "arabhi": (
                "**Arabhi** is an **Audava-Sampurna** raga. Its ascending scale is pentatonic (5 notes: `S R2 M1 P D2 S`), "
                "omitting Ga and Ni, while its descending scale is fully heptatonic (7 notes: `S N3 D2 P M1 G3 R2 S`)."
            )
        }
        ...
        if len(target_types) >= 2:
            response_parts.append(
                ...
                "> Note: The scale type may differ between ascending (arohana) and descending (avarohana) movements. A raga can be asymmetric (e.g. pentatonic ascending, heptatonic descending).\n"
            )

        for t_type in target_types:
            r_list = classified[t_type]
            if not r_list:
                continue
            
            note_count_desc = "5-Note Pentatonic Scale" if t_type == "Audava" else ("6-Note Hexatonic Scale" if t_type == "Shadava" else "7-Note Heptatonic Scale")
            response_parts.append(f"### {t_type} Ragas ({note_count_desc})\n")
            
            headers = ["Raga Name", "Type", "Parent Melakarta", "Arohana", "Avarohana"]
            table_header = "| " + " | ".join(headers) + " |\n"
            table_divider = "| " + " | ".join([":---"] * len(headers)) + " |\n"
            table_rows = ""
            for r in sorted(r_list, key=lambda x: x["name"]):
                parent_desc = r.get("parent", r.get("melakarta_name", "N/A"))
                table_rows += f"| **{r['name']}** | {r.get('type', 'Janya')} | {parent_desc} | `{r.get('arohana')}` | `{r.get('avarohana')}` |\n"
                
            response_parts.append(table_header + table_divider + table_rows)
            
            explanation = ""
            if t_type == "Audava":
                explanation = (
                    "\n**Audava** scale types omit exactly two swaras (Varja swaras) from the parent Melakarta scale. "
                    "They are known for their clear, direct, and immediately recognizable melodic identity.\n\n"
                    "*Note: Certain Audava-derived ragas like **Bilahari** and **Arabhi** utilize 5 notes in ascending scale but "
                    "re-introduce missing notes to use all 7 notes descending, classifying them as Audava-Sampurna.*"
                )
            elif t_type == "Shadava":
                explanation = (
                    "\n**Shadava** scale types omit exactly one swara (Varja swara) from the parent Melakarta scale, "
                    "creating a hexatonic scale structure with six active notes.\n\n"
                    "*Note: Ragas like **Kamboji** omit Nishadha ascending (6 notes) but use it descending (7 notes), "
                    "classifying them as Shadava-Sampurna.*"
                )
            elif t_type == "Sampurna":
                explanation = (
                    "\n**Sampurna** scale types include all seven swaras (heptatonic) in both ascending and descending movements in a linear progression.\n\n"
                    "*Note: Ragas like **Bhairavi** are Sampurna in both directions but utilize different variants of Dhaivata (D2 ascending, D1 descending) "
                    "between ascending and descending movements, creating an asymmetric scale structure.*"
                )
                
            response_parts.append(explanation + "\n")
            
        # Add the curated Asymmetric & Special Ragas section at the bottom for completeness
        response_parts.append("### Asymmetric & Special Ragas (Bhashanga, Vakra, and Audava-Sampurna)\n")
        response_parts.append("These ragas use asymmetric scales (different note counts ascending vs descending), zigzag (Vakra) progressions, or foreign notes (Bhashanga):\n\n")
        
        headers = ["Raga Name", "Arohana (Ascending)", "Avarohana (Descending)", "Scale Classification & Explanation"]
        asym_header = "| " + " | ".join(headers) + " |\n"
        asym_divider = "| " + " | ".join([":---"] * len(headers)) + " |\n"
        asym_rows = ""
        for r in sorted(classified["Asymmetric"], key=lambda x: x["name"]):
            name_lower = r["name"].lower()
            explanation_note = "Asymmetric scale type."
            for k, val in sorted(SPECIAL_RAGAS_NOTES.items(), key=lambda x: len(x[0]), reverse=True):
                if k in name_lower:
                    explanation_note = val
                    break
            asym_rows += f"| **{r['name']}** | `{r.get('arohana')}` | `{r.get('avarohana')}` | {explanation_note} |\n"
            
        response_parts.append(asym_header + asym_divider + asym_rows)
        
        combined_text = "\n".join(response_parts).strip()
        return combined_text, "knowledge_base"

    # ── LIST_RAGAS_AND_SCALES Interceptor ────────────────────────────────────
    if intent == "LIST_RAGAS_AND_SCALES":
        return (
            "## Beautiful Carnatic Ragas & Their Scales\n\n"
            "Here is a curated list of representative, highly popular Carnatic ragas highlighting their scale structures, ascending (arohana) and descending (avarohana) patterns, and musicological characteristics:\n\n"
            "| Raga Name | Scale Type | Arohana (Ascending Scale) | Avarohana (Descending Scale) | Key Characteristics & Mood |\n"
            "|---|---|---|---|---|\n"
            "| **Mohanam** | Audava (Pentatonic) | `S R2 G3 P D2 S` | `S D2 P G3 R2 S` | Auspicious, bright, evoking peace, beauty, and devotion. |\n"
            "| **Hindolam** | Audava (Pentatonic) | `S G2 M1 D1 N2 S` | `S N2 D1 M1 G2 S` | Meditative, peaceful, deeply devotional (omits Ri and Pa). |\n"
            "| **Hamsadhwani** | Audava (Pentatonic) | `S R2 G3 P N3 S` | `S N3 P G3 R2 S` | Bright, energetic, traditionally used to invoke Lord Ganesha. |\n"
            "| **Kalyani** | Heptatonic (Sampurna) | `S R2 G3 M2 P D2 N3 S` | `S N3 D2 P M2 G3 R2 S` | Luminous, majestic, utilizing Prati Madhyamam (M2). |\n"
            "| **Sankarabharanam** | Heptatonic (Sampurna) | `S R2 G3 M1 P D2 N3 S` | `S N3 D2 P M1 G3 R2 S` | Grand, royal, corresponding to the Western Major Scale. |\n"
            "| **Mayamalavagowla** | Heptatonic (Sampurna) | `S R1 G3 M1 P D1 N3 S` | `S N3 D1 P M1 G3 R1 S` | Symmetrical beginner training scale codified by Purandaradasa. |\n"
            "| **Bhairavi** | Asymmetric (Bhashanga) | `S R2 G2 M1 P D2 N2 S` | `S N2 D1 P M1 G2 R2 S` | Majestic, uses Chatusruti Dhaivata (D2) ascending and Suddha (D1) descending. |\n"
            "| **Bilahari** | Asymmetric (Audava-Sampurna) | `S R2 G3 P D2 S` | `S N3 D2 P M1 G3 R2 S` | 5 notes ascending (omits Ma, Ni), 7 notes descending. Joyous, heroic. |\n\n"
            "> **Scale Asymmetry Note:** Ragas like *Bilahari* and *Bhairavi* show that scale structure is not always symmetric between ascent (arohana) and descent (avarohana). A raga can omit notes in one direction or use different swara variants altogether to establish its unique melodic flavor (*raga swaroopa*)."
        ), "knowledge_base"
    ...
    if intent == "TALA_QUERY" or (intent not in ["TALA_COMPARISON", "COMPARISON"] and "tala" in query_lower and any(re.search(rf"\b{re.escape(t)}\b", query_lower) for t in ["adi", "rupaka", "ata", "triputa", "eka", "chapu", "misra", "khanda", "dhruva", "matya", "jhampa"])):
        # Check parent talas
        tala_info = None
        if "dhruva" in query_lower:
            tala_info = {
                "name": "Dhruva Tala",
                "angas": "1 Laghu (I) + 1 Drutam (O) + 2 Laghus (I I)",
                "structure": "I O I I",
                "beats": "14 (in standard Chatusra Jati: 4 + 2 + 4 + 4)",
                "description": "Dhruva Tala is the first of the Suladi Sapta Talas. It is characterized by its long cycle, featuring a sequence of a Laghu, a Drutam, and two more Laghus. It represents a grand and majestic rhythmic flow."
            }
        elif "matya" in query_lower:
            tala_info = {
                "name": "Matya Tala",
                "angas": "1 Laghu (I) + 1 Drutam (O) + 1 Laghu (I)",
                "structure": "I O I",
                "beats": "10 (in standard Chatusra Jati: 4 + 2 + 4)",
                "description": "Matya Tala is the second of the Suladi Sapta Talas. Its symmetric structure consists of a Laghu, followed by a Drutam, and ending with another Laghu."
            }
        elif "rupaka" in query_lower and not "sapta" in query_lower:
            tala_info = {
                "name": "Rupaka Tala",
                "angas": "1 Drutam (O) + 1 Laghu (I)",
                "structure": "O I",
                "beats": "6 (in standard Chatusra Jati: 2 + 4) or 3 beats in common practice",
                "description": "Rupaka Tala is the third of the Suladi Sapta Talas. In classical musicology, it is structured as a Drutam followed by a Laghu. In common practice, it is counted as 3 beats (with a clap and two finger counts)."
            }
        elif "jhampa" in query_lower:
            tala_info = {
                "name": "Jhampa Tala",
                "angas": "1 Laghu (I) + 1 Anudrutam (U) + 1 Drutam (O)",
                "structure": "I U O",
                "beats": "10 (in standard Misra Jati: 7 + 1 + 2)",
                "description": "Jhampa Tala is the fourth of the Suladi Sapta Talas. It uniquely features the Anudrutam limb (1 beat tap) positioned between a variable Laghu and a Drutam. It is traditionally performed in Misra Jati (7 beats Laghu)."
            }
        elif "triputa" in query_lower:
            tala_info = {
                "name": "Triputa Tala",
                "angas": "1 Laghu (I) + 2 Drutams (O O)",
                "structure": "I O O",
                "beats": "7 (in standard Tisra Jati: 3 + 2 + 2) or 8 beats as Adi Tala (in Chatusra Jati: 4 + 2 + 2)",
                "description": "Triputa Tala is the fifth of the Suladi Sapta Talas. Its structure consists of a Laghu followed by two Drutams. In Chaturasra Jati (4 beats Laghu), it becomes the ubiquitous Adi Tala (8 beats), the most widely used rhythmic cycle in Carnatic music."
            }
        elif "ata" in query_lower and not "sapta" in query_lower:
            tala_info = {
                "name": "Ata Tala",
                "angas": "2 Laghus (I I) + 2 Drutams (O O)",
                "structure": "I I O O",
                "beats": "14 (in standard Khanda Jati: 5 + 5 + 2 + 2)",
                "description": "Ata Tala is the sixth of the Suladi Sapta Talas. It is a grand and long cycle, traditionally performed in Khanda Jati (5 beats Laghu), resulting in a 14-beat cycle. It is famously used for heavy, majestic Varnams (such as the Viriboni Varnam in Bhairavi)."
            }
        elif "eka" in query_lower:
            tala_info = {
                "name": "Eka Tala",
                "angas": "1 Laghu (I) only",
                "structure": "I",
                "beats": "4 (in standard Chatusra Jati) or variable based on Jati",
                "description": "Eka Tala is the seventh of the Suladi Sapta Talas. It is the simplest tala, consisting of a single Laghu limb. Its beat duration changes dynamically based on the Jati selected (e.g., Tisra Jati = 3 beats, Chatusra Jati = 4 beats)."
            }

        if tala_info:
            return (
                f"### Structured Music Theory Response: {tala_info['name']} (Suladi Sapta Tala System)\n\n"
                f"- **Definition:**\n"
                f"**{tala_info['name']}** is one of the seven parent rhythmic cycles (Talas) in the traditional **Suladi Sapta Tala** system of Carnatic musicology.\n\n"
                f"- **Anatomy & Rhythmic Structure:**\n"
                f"  - **Angas (Limbs):** {tala_info['angas']}\n"
                f"  - **Rhythmic Notation:** `{tala_info['structure']}`\n"
                f"  - **Standard Beat Count:** {tala_info['beats']} beats\n\n"
                f"- **Detailed Description:**\n"
                f"{tala_info['description']}\n\n"
                f"- **The Suladi Sapta Tala Context:**\n"
                f"Under the Sapta Tala framework, each of the 7 parent talas can be combined with the 5 rhythmic **Jaatis** (Tisra = 3, Chatusra = 4, Khanda = 5, Misra = 7, Sankirna = 9) which determine the length of the Laghu (I). This generates the 35 core talas of Carnatic music.\n\n"
                f"- **Exam Notes:**\n"
                f"Be prepared to write down the angas, structure, and compute the total beats for any jaati of {tala_info['name']}. For example, in Tisra Jati, the Laghu (I) is 3 beats."
            ), "ft"
        ...
        for t in db_loader.TALAS:
            ...
            short_name = t_name.replace(" tala", "")
            if re.search(rf"\b{re.escape(t_name)}\b", query_lower) or re.search(rf"\b{re.escape(short_name)}\b", query_lower):
                tala_match = t
                break
    ...
    if intent == "THEORY_CONCEPT_QUERY":
        ...
        # Check custom high-fidelity templates first
        q_l = query_lower
        
        # 1. Concert Structure
        if any(w in q_l for w in ["concert structure", "concert format", "concert sequence", "katcheri paddhati", "katcheri structure", "katcheri format", "ariyakudi"]):
            return (
                ...
                "- **Exam Notes:**\n"
                "Be prepared to name Ariyakudi Ramanuja Iyengar as the father of the modern *Katcheri Paddhati* (Ariyakudi Format) and outline the order of components from Varnam to Mangalam."
            ), "ft"

        # 2. Suladi Sapta Talas
        if any(w in q_l for w in ["sapta talas", "sapta tala", "suladi"]):
            return (
                "### Structured Music Theory Response: Suladi Sapta Talas\n\n"
                "- **Definition:**\n"
                "The **Suladi Sapta Tala** system is the foundational rhythmic classification scheme in Carnatic musicology, organizing rhythmic cycles (talas) into seven primary families using three basic constituent limbs (Angas).\n\n"
                "- **The Seven Parent Talas & Their Notation:**\n"
                "1. **Dhruva Tala** (Angas: `I O I I` — Laghu, Drutam, Laghu, Laghu)\n"
                "2. **Matya Tala** (Angas: `I O I` — Laghu, Drutam, Laghu)\n"
                "3. **Rupaka Tala** (Angas: `O I` — Drutam, Laghu)\n"
                "4. **Jhampa Tala** (Angas: `I U O` — Laghu, Anudrutam, Drutam)\n"
                "5. **Triputa Tala** (Angas: `I O O` — Laghu, Drutam, Drutam) — *Adi Tala is Chaturasra Jati Triputa Tala (`I4 O O`)*\n"
                "6. **Ata Tala** (Angas: `I I O O` — Laghu, Laghu, Drutam, Drutam)\n"
                "7. **Eka Tala** (Angas: `I` — Laghu only)\n\n"
                "- **Tala Construction & Math (The 35 Talas):**\n"
                "Each of the 7 parent talas has a variable limb called the **Laghu (I)**. The Laghu can take 5 different beat counts based on the rhythmic family (**Jaati**):\n"
                "- **Tisra Jati:** 3 beats (tap + 2 finger counts)\n"
                "- **Chatusra Jati:** 4 beats (tap + 3 finger counts)\n"
                "- **Khanda Jati:** 5 beats (tap + 4 finger counts)\n"
                "- **Misra Jati:** 7 beats (tap + 6 finger counts)\n"
                "- **Sankirna Jati:** 9 beats (tap + 8 finger counts)\n\n"
                "By multiplying the 7 parent talas by the 5 Jatis, we generate the core **35 Sapta Talas** of Carnatic music.\n\n"
                "- **Key Points:**\n"
                "- **Anudrutam (U):** 1 beat (clap/tap only). Constant.\n"
                "- **Drutam (O):** 2 beats (clap + wave of the hand). Constant.\n"
                "- **Laghu (I):** Variable beats (clap + counting of fingers). Dynamic.\n\n"
                "- **Exam Notes:**\n"
                "Be prepared to state the beat calculations for any given Tala and Jaati. For example, *Ata Tala* is `I I O O`. In *Tisra Jati Ata Tala*, the Laghu is 3 beats, so the count is `3 + 3 + 2 + 2 = 10` beats."
            ), "ft"

        # 3. Tala Construction / Structure
        if "construction" in q_l or "structure" in q_l:
            if "tala" in q_l or "talam" in q_l or "thala" in q_l:
                return (
                    "### Structured Music Theory Response: Tala Construction & Rhythmic Anatomy\n\n"
                    "- **Definition:**\n"
                    "**Tala Construction** refers to the musicological principles and structural components (Angas, Jatis, Kriyas) used to build, measure, and perform rhythmic cycles in Carnatic music.\n\n"
                    "- **The Core Components of Tala Construction:**\n"
                    "1. **Angas (Limbs):** The components that make up a Tala cycle. The three primary Angas are:\n"
                    "   - **Anudrutam (U):** 1 beat (a simple downward hand tap).\n"
                    "   - **Drutam (O):** 2 beats (a hand tap followed by an outward wave/flip of the palm).\n"
                    "   - **Laghu (I):** Variable beats (a hand tap followed by finger counts starting from the little finger).\n"
                    "2. **Jaati (Rhythmic Families):** Dictates the number of beats in the Laghu section. The five Jatis are Tisra (3), Chatusra (4), Khanda (5), Misra (7), and Sankirna (9).\n"
                    "3. **Kriyas (Rhythmic Actions):** The physical gestures used to count the Tala, split into *Sashabda Kriya* (sounded gestures like claps/taps) and *Nishabda Kriya* (silent gestures like waves/finger counts).\n\n"
                    "- **Mathematical Expansion (The 175 Tala Matrix):**\n"
                    "Tala construction expands from the 7 parent talas to 35 talas via the 5 Jatis. It is further expanded to **175 talas** by applying the 5 **Gatis (or Nadai)**, which dictate the micro-tempo/pulses per beat (Tisra = 3, Chatusra = 4, Khanda = 5, Misra = 7, Sankirna = 9).\n\n"
                    "- **Key Points:**\n"
                    "- **Laya (Tempo):** The speed of the cycle, categorized as *Vilamba* (slow), *Madhyama* (medium), and *Druta* (fast).\n"
                    "- **Avartanam:** One complete execution of the rhythmic cycle.\n"
                    "- **Akshara Kala:** The duration of a single unit beat.\n\n"
                    "- **Exam Notes:**\n"
                    "Be ready to define the six classical limbs (**Shadangas**) historically used in ancient talas: Anudrutam, Drutam, Laghu, Guru (8 beats), Plutam (12 beats), and Kakapadam (16 beats)."
                ), "ft"

        # 4. Tillana Template
        if "tillana" in q_l or "thillana" in q_l:
            return (
                "### Structured Music Theory Response: Tillana (Rhythmic Concluding Form)\n\n"
                "- **Definition:**\n"
                "A **Tillana** is a lively, fast-paced, and highly rhythmic compositional form characterized by the dominant use of non-lexical solfa syllables (solkattu/vocables), performed near the conclusion of a Carnatic music concert or a Bharatanatyam recital.\n\n"
                "- **Structure of a Tillana:**\n"
                "- **Pallavi:** Written entirely using rhythmic syllables like *Dheem*, *Ta*, *Na*, *Nom*, *Dirana*, set to a fast, energetic tempo.\n"
                "- **Anupallavi:** Continues the rhythmic vocables, often expanding into the upper octave register.\n"
                "- **Charanam:** Introduces a brief section of meaningful lyrics (Sahitya) containing a dedication to a deity, patron, or guru, followed by the composer's signature (**Mudra**).\n\n"
                "- **Aesthetic Significance:**\n"
                "Unlike the Kriti, which is lyric-centric, the Tillana is drum-syllable-centric. It focuses on absolute rhythmic precision, speed, and melodic charm. It creates a joyous, high-energy, and celebratory atmosphere, making it a perfect climax for concerts and dance recitals.\n\n"
                "- **Famous Composers & Examples:**\n"
                "- **Lalgudi Jayaraman:** Composed legendary Tillanas in ragas like *Dhanasri*, *Desh*, and *Revati*.\n"
                "- **M. Balamuralikrishna:** Known for Tillanas in *Kuntalavarali* and *Brindavana Saranga*.\n"
                "- **Swathi Thirunal:** Composed classic Tillanas in Hindustani-influenced ragas.\n\n"
                "- **Exam Notes:**\n"
                "Contrast the Tillana with the Varnam (Varnam is a technical warm-up featuring swaras and lyrics in equal parts; Tillana is a fast-paced concluding piece dominated by drum syllables)."
            ), "ft"

        # 5. Kriti Template
        if "kriti" in q_l or "krithi" in q_l:
            return (
                "### Structured Music Theory Response: Kriti (Classical Compositional Form)\n\n"
                "- **Definition:**\n"
                "A **Kriti** is the primary, most highly developed, and standard classical art-song composition in Carnatic sangeetham. It is a devotional or philosophical composition that beautifully balances literary lyrics (Sahitya) and melodic framework (Raga).\n\n"
                "- **Structural Divisions of a Kriti:**\n"
                "1. **Pallavi:** The opening section or refrain that contains the core thematic statement of the song.\n"
                "2. **Anupallavi:** The second section that complements the Pallavi, usually sung in the higher octave range and introducing melodic contrast.\n"
                "3. **Charanam:** The final and longest section that carries the main narrative, concluding with a melodic transition that resolves back to the Pallavi refrain.\n\n"
                "- **Compositional Elements:**\n"
                "- **Sangatis:** Variations of a melodic line introduced to expand the raga's flavor on a single set of lyrics (popularized by Tyagaraja).\n"
                "- **Mudra:** The composer's unique signature woven into the final lines of the Charanam (e.g. *Guruguha* for Dikshitar, *Shri Rama* or *Tyagaraja* for Tyagaraja, *Syamakrishna* for Syama Sastri).\n"
                "- **Chittaswaram / Madhyamakala Sahityam:** Optional pre-composed sections in medium/fast speed added to enhance the rhythmic and structural beauty of the Kriti.\n\n"
                "- **Kriti vs. Kirtana:**\n"
                "- **Kriti:** Focuses on musical complexity, sophisticated raga details, and serving as a launchpad for improvisation (Manodharma).\n"
                "- **Kirtana:** Primarily devotional, simple in melody, repetitive, and meant for group/congregational singing (bhajan style).\n\n"
                "- **Exam Notes:**\n"
                "Be prepared to describe the three sections of a Kriti and explain the difference between a Kriti and a Kirtana for a viva exam."
            ), "ft"
        ...
        generic_terms = ["swara", "raga", "tala", "shruti", "shruthi", "ragam", "raaga", "thala", "sangeetam", "sangeetham", "music", "system", "swaras", "ragas", "talas", "rtp", "gamaka", "gamakas", "gamakam", "gamakams", "kriti", "krithi", "tillana", "thillana", "ragamalika", "mangalam", "tukkada", "pallavi", "kalpana swara", "kalpanaswaram", "swarakalpana"]
        ...
        return (
            ...
            "- **Definition:**\n"
            "Ragam Tanam Pallavi (RTP) is the premier, most elaborate improvisational form in a Carnatic concert, showcasing the absolute peak of a musician's creative, melodic, and rhythmic mastery.\n\n"
            "- **Explanation:**\n"
            "RTP is split into three distinct, progressive phases:\n"
            "1. **Ragam (Alapana):** A free-form, unmetered melodic improvisation exploring the raga's personality, moving through the lower, middle, and upper octaves.\n"
            "2. **Tanam:** A rhythmic, pulsed improvisation without a strict drum beat (mridangam), using rhythmic vocables (like *Ananta*, *Ta*, *Na*, *Nom*) to create a steady tempo.\n"
            "3. **Pallavi:** A highly structured, pre-composed short line of lyrics (Sahitya) set to a specific Tala. The artist performs complex rhythmic variations such as speed changes (Trikala - Anuloma/Pratiloma), shift of the starting beat (Eduttu shift), and solfa swara improvisation (Kalpanaswaram/Ragamalika).\n\n"
            "- **Musicological Context:**\n"
            "Historically, performing a full RTP is considered the ultimate test of a Carnatic vocalist's or instrumentalist's technical precision. It is the centerpiece of any major concert (*Katcheri*).\n\n"
            "- **Key Points:**\n"
            "- **Ragam:** Expounds raga identity without rhythmic metre.\n"
            "- **Tanam:** Features steady pulse (*laya*) using specific rhythmic vocal patterns.\n"
            "- **Pallavi:** The word stands for **Pa** (Pada - words), **La** (Laya - rhythm), and **Vi** (Vinyasa - variations).\n\n"
            "- **Exam Notes:**\n"
            "Be prepared to explain the components of a Pallavi (Prathamanga and Dvitayanga divided by the *Arudi* or resting point) and how Trikala is executed (singing the melody in double and quadruple speeds while keeping the tala count constant)."
        ), "ft"
    ...
    # ── 1.7 Dynamic API keys check (Gemini/OpenAI) ───────────────────────────
    if use_llm:
        gemini_key, openai_key = get_api_keys()
        if gemini_key:
            try:
                answer = _call_gemini(query, norm, gemini_key)
                if _is_real_answer(answer):
                    log.info("synthesis_method=gemini  len=%d", len(answer))
                    return answer, "gemini"
            except Exception as e:
                log.warning("Gemini API call failed: %s. Trying fallbacks...", e)
                
        if openai_key:
            try:
                answer = _call_openai(query, norm, openai_key)
                if _is_real_answer(answer):
                    log.info("synthesis_method=openai  len=%d", len(answer))
                    return answer, "openai"
            except Exception as e:
                log.warning("OpenAI API call failed: %s. Trying fallbacks...", e)

    # ── 1.8 Rule-based fallback ───────────────────────────────────────────────
