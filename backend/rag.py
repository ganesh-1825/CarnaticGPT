...
def execute_rag_pipeline(query: str) -> dict:
    ...
    Delegates directly to the unified answer_question pipeline in backend/services/retrieval.py.
    """
    logger.info(f"Executing unified RAG Pipeline for query: '{query}'")
    from backend.services.retrieval import answer_question
    from backend.services.query_router import _extract_raga
    
    result = answer_question(query)
    
    score = result.get("top_confidence", 0.0)
    if "low_confidence_fallback" in result.get("route", "") or score < 15.0 or result.get("confidence_label") == "No Evidence":
        return {
            "response": "Low confidence answer",
            "confidence": "Low Confidence",
            "detected_raga": result.get("raga_name") or _extract_raga(query),
            "citations": []
        }
    ...
    for c in result.get("citations", []):
        citations.append({
            ...
            "text": c.get("excerpt") or c.get("text", ""),
            "source": c.get("source", ""),
            "book_name": c.get("book_name", ""),
            "page": c.get("page_number", 1),
            "score": c.get("confidence", 0.0) / 100.0 if c.get("confidence", 0.0) > 1.0 else c.get("confidence", 0.0),
            "confidence": c.get("confidence_label", "medium").title() + " Confidence"
        })
        
    return {
        "response": result["answer"],
        "confidence": result["confidence_label"].title() + " Confidence",
        "citations": citations,
        "detected_raga": result.get("raga_name") or _extract_raga(query)
    }
